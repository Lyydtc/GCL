...
class CrossAttention(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.linear_q = nn.Linear(embedding_size, self.args.n_heads * embedding_size, bias=args.msa_bias)
        self.linear_k = nn.Linear(embedding_size, self.args.n_heads * embedding_size, bias=args.msa_bias)
        self.outlayer = nn.Linear(self.args.n_heads * math.ceil(self.args.n_max_nodes * self.args.ratio), embedding_size)
    ...
    def forward(self, embeddings_i, mask_i, embeddings_j, mask_j, mask_ij=None):
        ...
        k_i = self.linear_k(embeddings_i).view(node_size_i, -1, self.n_heads, self.embedding_size).transpose(-2,
                                                                                                             -3).transpose(
            -1, -2)
        q_j = self.linear_q(embeddings_j).view(node_size_j, -1, self.n_heads, self.embedding_size).transpose(-2, -3)
        k_j = self.linear_k(embeddings_j).view(node_size_j, -1, self.n_heads, self.embedding_size).transpose(-2,
                                                                                                             -3).transpose(
            -1, -2)

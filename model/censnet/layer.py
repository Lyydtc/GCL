...
class GraphConvolution(Module):
    ...
    def forward(self, H_v, H_e, adj_e, adj_v, T):
        if self.node_layer:
            multiplier1 = torch.spmm(T, torch.diag((H_e @ self.p.t()).t()[0])) @ T.to_dense().t().to(self.args.device)
            ...
            adj_v = adj_v.to_dense().to(self.args.device) + torch.eye(H_v.shape[0]).to(self.args.device)
            ...
            multiplier2 = torch.spmm(T.t(), torch.diag((H_v @ self.p.t()).t()[0])) @ T.to_dense().to(self.args.device)
            ...
            adj_e = adj_e.to_dense().to(self.args.device) + torch.eye(H_e.shape[0]).to(self.args.device)
